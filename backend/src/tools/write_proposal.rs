...
impl WriteProposal {
    pub(crate) fn is_write(name: &str) -> bool {
        matches!(
            name,
            "project.replace_text" | "project.create_file"
        )
    }
    ...
    pub(crate) async fn prepare(
        ...
        tokio::task::spawn_blocking(move || {
            Self::prepare_sync(&root, &name, &arguments)
        })
        .await?
    }
    ...
    fn prepare_sync(
        ...
        let object = arguments
            .as_object()
            .context("Arguments JSON invalides")?;
        ...
        let (previous, replacement) = if create {
            ...
            let text = str::from_utf8(&original.bytes)
                .context("Fichier non UTF-8")?;
            ...
            {
                bail!(
                    "Le texte à remplacer doit être unique et différent"
                );
            }
